...
class PlayActivity : AppCompatActivity() {
    ...
    private fun startTimer(mTimeLeftInMillis: Long) {
        ...
        mCountDownTimer = object : CountDownTimer(mTimeLeftInMillis, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                val m = mTimeLeftInMillis - millisUntilFinished
                updateOurCountDownText(m)
            }
    ...
    private fun startTimer2() {
        ...
        mCountDownTimer2 = object : CountDownTimer(mTimeLeftInMillis, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                val k = START_TIME_IN_MILLIS - millisUntilFinished
                updateOurCountDownText2(k)
            }
    ...
    private fun winner() {
        ...
        builder.setTitle(resources.getString(R.string.wintittledialog) + tvCountAllSeconds!!.text.toString())
            .setCancelable(false)
            .setPositiveButton(
                resources.getString(R.string.vihod)
            ) { dialog, which -> finish() }
            .setNeutralButton(
                resources.getString(R.string.igrat)
            ) { dialog, which ->
                finish()
    ...
    private fun loosing() {
        ...
        builder.setTitle(resources.getString(R.string.loosetittledialog) + redScoreCount!!.text.toString())
            .setCancelable(false)
            .setPositiveButton(
                resources.getString(R.string.vihod)
            ) { dialog, which -> finish() }
